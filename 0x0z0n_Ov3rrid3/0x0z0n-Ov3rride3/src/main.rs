k#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]
...
// --- [ MODIFIED ] PROCESS INTERCEPTOR (Scrapes Kernel for Full Command) ---
#[tauri::command]
    ...
    if let Ok(output) = Command::new("nvidia-smi").args(["--query-compute-apps=pid,process_name,used_memory", "--format=csv,noheader,nounits"]).output() {
        ...
        for line in stdout.lines() {
            ...
            if parts.len() == 3 {
                ...
                // DEEP SCAN: Scrape /proc/<pid>/cmdline to get the exact payload execution string
                let mut full_cmd = name.to_string();
                if let Ok(cmdline) = fs::read_to_string(format!("/proc/{}/cmdline", pid)) {
                    // Linux cmdline separates args with Null bytes (\0), we replace them with spaces
                    let parsed = cmdline.replace('\0', " ").trim().to_string();
                    if !parsed.is_empty() {
                        full_cmd = parsed;
                    }
                }
                ...
                processes.push(serde_json::json!({
                    "pid": pid, 
                    "name": name, 
                    "full_cmd": full_cmd, // Pushing the deep-scanned payload to the UI
                    "memory": memory
                }));
            }
        }
...
// --- [ KALI NATIVE TERMINAL EXECUTION ENGINE ] ---
...
// --- [ MODIFIED ] AUTOMATED TMUX DASHBOARD ---
#[tauri::command]
fn launch_soc_monitor() -> Result<String, String> {
    // We use the Tmux 'Space' keyword to completely bypass Linux terminal quote-escaping bugs
    let tmux_script = "\
