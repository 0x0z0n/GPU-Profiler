...
const powerModes = [
  { name: "ECO_STEALTH", watts: 45, color: "var(--success-green)", class: "text-green" },
  { name: "NOMINAL_YIELD", watts: 70, color: "var(--cyan-neon)", class: "text-cyan" },
  { name: "KINETIC_BURST", watts: 95, color: "#ffaa00", class: "text-orange" },
  { name: "OVERRIDE_MAX", watts: 115, color: "var(--crimson-neon)", class: "text-crimson" }
];
...
// --- FAVORITES (Replaces JSON) ---
function loadFavorites() {
  const favs = JSON.parse(localStorage.getItem("z0n_favorites")) || [];
  appCombo.innerHTML = favs.length ? "" : "<option>None Selected</option>";
  favs.forEach(f => appCombo.innerHTML += `<option value="${f}">${f}</option>`);
}

function saveFavorite(path) {
  let favs = JSON.parse(localStorage.getItem("z0n_favorites")) || [];
  if (!favs.includes(path)) {
    favs.unshift(path);
    if (favs.length > 10) favs.pop();
    localStorage.setItem("z0n_favorites", JSON.stringify(favs));
    loadFavorites();
  }
  appCombo.value = path;
}
...
document.getElementById("btn-launch-cmd").addEventListener("click", async () => {
  const payload = document.getElementById("cmd-entry").value.trim();
  if (!payload) return log("ERR: NO PAYLOAD PROVIDED", true);
  try {
    log(`LAUNCH [02] -> ${payload}`);
...
document.getElementById("btn-apply-power").addEventListener("click", async () => {
  ...
  log(`INITIATING HARDWARE OVERRIDE -> ${watts}W`);
  try {
...
brightSlider.addEventListener("change", async (e) => { // 'change' triggers on release
  const level = parseInt(e.target.value);
  brightStatus.innerText = `[ BRIGHTNESS ] // ${level * 10}%`;
  try { await invoke("set_brightness", { level: level / 10.0 }); } 
  catch (err) { log(err, true); }
});
...
async function updateTelemetry() {
  try {
    ...
    document.getElementById("nv-stats").innerText = `${data.nvidia.temp}°C | ${data.nvidia.power}W | ${data.nvidia.vram_used}MB / ${data.nvidia.vram_total}MB`;
