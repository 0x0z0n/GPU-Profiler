...
fn set_brightness(level: f32) -> Result<String, String> {
    ...
    if output.status.success() { Ok(format!("SET: {}%", (level * 100.0) as i32)) } 
    else { Err("ERR: XRANDR FAILED".to_string()) }
}
...
fn browse_file() -> Result<String, String> {
    let output = Command::new("zenity")
        .args(["--file-selection", "--title=z0n // SELECT TARGET", "--filename=/usr/bin/", "--modal"])
        .output().map_err(|e| e.to_string())?;
    if output.status.success() { Ok(String::from_utf8_lossy(&output.stdout).trim().to_string()) } 
    else { Err("ABORTED".to_string()) }
}

// [05] SPECIAL: Persistent NVIDIA Shell
#[tauri::command]
fn spawn_nvidia_terminal() -> Result<String, String> {
    ...
    let terminal_cmd = format!("x-terminal-emulator -e bash -c \"export {}; echo '>> NVIDIA GPU ENVIRONMENT ACTIVE'; exec bash\"", env_vars);
    Command::new("bash").arg("-c").arg(&terminal_cmd).spawn().map_err(|e| e.to_string())?;
    Ok("SUCCESS: SHELL SPAWNED".to_string())
}
